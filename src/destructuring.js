/** ******************************************************************************************************************
 * @file Describe what destructuring does.
 ...
 * @return {*}
 */
function destructure( lhs, rhs )
{
    switch ( lhs.type )
    {
        case Syntax.ObjectPattern:
            if ( !lhs.properties.length )
            {
                return void require_object_coercible( rhs );
            }

            require_object_coercible( rhs );
            lhs.properties.forEach( prop => destructure( prop, rhs ) );
            break;

        case Syntax.ArrayPattern:
            if ( !lhs.elements.length )
            {
                return void require_iterator( rhs );
            }

            const iter = require_iterator( rhs );
...
function iterator_destructure( el, iter )
{
    if ( el === null ) return;

    if ( el.type !== Syntax.ObjectPattern && el.type !== Syntax.ArrayPattern )
    {
        destructure( el, iter.value() );
    }

    const value = iter.next();
}

function require_object_coercible( node )
{

}

function require_iterator( node )
{
...
class Iterator
{
    ...
    next()
    {
        if ( this.sequence.has_value( this.index ) )
        {
            return { value: this.sequence.value( this.index ), done: false };
        }
...
{
    return node.type === Syntax.AssignmentExpression && ( node.left.type === Syntax.ObjectPattern || node.left.type === Syntax.ArrayPattern );
}

function isEmptyArrayLiteral( node )
{
    return node.type === Syntax.ArrayPattern && node.elements.length === 0;
}

function isEmptyObjectLiteral( node )
{
    return node.type === Syntax.ObjectPattern && node.properties.length === 0;
}
...
function visitNode( node, visitor, isExpr )
{
 ...
 *
 * @param {VariableDeclaration|AssignmentPattern|BaseNodeWithoutComments|Node} node - The node to flatten.
 * @param {function} visitor An optional visitor used to visit initializers.
 * @param {object} context The transformation context.
 * @param {string} level Indicates the extent to which flattening should occur.
 * @param {boolean} needsValue An optional value indicating whether the value from the right-hand-side of
 * the destructuring assignment is needed as part of a larger expression.
 * @param {function(Identifier, Expression, [BaseNodeWithoutComments.range]): Expression} createAssignmentCallback An optional callback used to create the assignment expression.
 */
 ...
 *
 * @param {VariableDeclaration|VariableDeclarator|Declaration} node The node to flatten.
 * @param {function} visitor An optional visitor used to visit initializers.
 ...
 * @return {Array<VariableDeclarator>}
 */
export function flattenDestructuringBinding( node, visitor, context, level, rval, hoistTempVariables, skipInitializer )
{
    ...
    {
        const temp = createTempVariable( /*recordTempVariable*/ void 0 );

        if ( hoistTempVariables )
        {
            const value = inlineExpressions( pendingExpressions );
            pendingExpressions = void 0;
            emitBindingOrAssignment( temp, value, /*location*/ void 0, /*original*/ void 0 );
        }
        else
        {
 ...
 * for the element.
 */
function flattenBindingOrAssignmentElement( flattenContext: FlattenContext,
