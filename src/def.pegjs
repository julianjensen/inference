...
prog =
	lines:( __ primary __ EOS )* __ EOF
    {
...
declaration =
	'declare' __ tp:decls __ def:symbol
    {
    	return { modifier: tp, symbol: def };
    }

symbol =
	r:( spec / func / array / constr ) { return r; }

func =
	n:name __ LPARENS params:list? RPARENS __ t:type
    {
    	return { name: n, params, ret: t };
    }
...
constr =
	nw:NEW? __ LPARENS __ params:list? __ RPARENS __ t:type
    {
    	const r = { name: 'constructor', params, ret: t };
        if ( nw ) r.usesNew = true;
        return r;
    }

list =
	__ head:symbol tail:( __ ',' __ symbol )*
    {
    	return [ head ].concat( tail.map( t => t[ 2 ] ) );
    }

name =
	id:Identifier __ opt:OPT?
    {
    	return ( opt || '?' ) + id; // { name: id, optional: opt === '?' };
    }

type =
	TYPE __ t:Identifier { return t; }

types =
	head:typename tail:( __ TYPESEP __ typename )+
    {
...
typename =
	tn:Identifier __ tpl:template
    {
        ...
        if ( tl && tl.length ) r.typeParameters = tl;

        return r;
    }
    /
    tn:Identifier { return tn; }

template =
	LANGLE __ tl:typelist __ RANGLE

typelist =
	h:type t:( __ ',' __ type )* { return [ h ].concat( t.map( _ => _[ 3 ] ) ); }
...
decls =
	m:( ConstToken / EnumToken / FunctionToken ) { return m[ 0 ]; }
